          ...
          T: TextureType<C>
{
    ...
    pub fn delete(self, state: &ContextState) {
        unsafe {
            ...
            state.sampler_units.0.unbind_texture(self.handle, T::bind_target(), &state.gl);
        }
    }
}
...
impl RawSamplerUnits {
    ...
    #[inline]
    pub unsafe fn bind_texture<'a, C, T>(&'a self, unit: u32, tex: &'a RawTexture<C, T>, gl: &'a Gl) -> RawBoundTexture<'a, C, T>
        where C: ColorFormat,
              T: 'a + TextureType<C>
    {
        #[inline(never)]
    ...
    unsafe fn unbind_texture(&self, handle: GLuint, target: GLuint, gl: &Gl) {
          ...
          T: TextureType<C>
{
        ...
        where I: Image<'b, C, T>
    {
        unsafe {
            ...
            assert!(level_int <= self.tex.num_mips().to_glint());

            if level_int >= self.tex.num_mips() as GLint {
                self.tex.num_mips = level.try_increment();
                self.gl.TexParameteri(T::bind_target(), gl::TEXTURE_MAX_LEVEL, self.tex.num_mips() as GLint);
            }
            ...
            let for_each_variant = |func: fn(&Gl, GLenum, GLint, GLsizei, GLsizei, GLsizei, *const GLvoid)| {
                ...
                match image {
                    Some(image_data) => image_data.variants(|image_bind, data| func(self.gl, image_bind, level_int, width, height, depth, data.as_ptr() as *const GLvoid)),
